...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'upload_ad' && isset($_FILES['ad_media'])) {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            $uploadError = 'File type not allowed. Use JPEG/PNG/GIF/WEBP/MP4/WEBM/OGG.';
        } elseif ($file['size'] > $maxSize) {
            $uploadError = 'File too large (max 50 MB).';
        } else {
            ...
            // Remove old ad files
            foreach (glob($destDir . 'current_ad.*') as $old) unlink($old);

            if (move_uploaded_file($file['tmp_name'], $destFile)) {
                file_put_contents($destDir . 'manifest.json', json_encode([
                    'filename' => 'current_ad.' . $ext,
                    'mime'     => $file['type'],
                    'updated'  => date('Y-m-d H:i:s'),
                ]));
                $uploadMsg = 'Ad media updated successfully.';
            } else {
                ...
                <tr>
                  ...
                  <td>
                    <span class="password-mask"
                          onclick="this.textContent = this.textContent === '••••' ? '<?= htmlspecialchars($row['password']) ?>' : '••••'"
    ...
    <div class="panel" id="ads">
      ...
      <div class="ad-manager">
        ...
        <form method="POST" enctype="multipart/form-data">
          ...
          <div class="ad-upload-zone" id="dropzone">
            <input type="file" name="ad_media" id="adFile"
                   accept="image/jpeg,image/png,image/gif,image/webp,video/mp4,video/webm,video/ogg"
                   onchange="updateDropLabel(this)">
            <div class="upload-icon">📁</div>
            <div class="upload-label" id="dropLabel">Drop ad media here</div>
            <div class="upload-hint">Images: JPG · PNG · GIF · WEBP &nbsp;|&nbsp; Videos: MP4 · WEBM · OGG<br>Max 50 MB</div>
            <br>
            <button class="btn btn-accent" type="submit" style="pointer-events:none;">Upload &amp; Set Live</button>
          </div>
        </form>
...
document.getElementById('dropzone').addEventListener('click', function(e) {
  if (e.target.tagName !== 'BUTTON') {
    document.getElementById('adFile').click();
  }
});
</script>
</body>
